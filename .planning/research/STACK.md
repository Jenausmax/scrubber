...
**Domain:** Cross-platform Electron desktop app — mp4 → audio → Whisper transcription (local + cloud) → LLM analysis
**Researched:** 2026-05-27
**Confidence:** HIGH (versions verified via npm registry; architecture patterns verified against multiple shipping OSS apps)
...
> **Русский язык:** для локального режима по умолчанию ставь модель **`large-v3`** (для non-English она заметно точнее turbo/medium; turbo деградирует на русском). Если ресурсов мало — `medium` как компромисс (~5 ГБ RAM). Дай в настройках выбор модели + квантизацию (q5_0/q8_0). Включай **VAD-препроцессинг** (`large-v3` склонна «выдумывать» текст на длинных паузах) и прокидывай язык `ru` явно (не полагайся на автодетект).
...
npm create @quick-start/electron@latest scrubber
...
# Dev
npm install -D electron@^42 electron-vite electron-builder typescript
...
**Если приоритет — простота дистрибутива и надёжность (рекомендуется для v1):**
...
**Если позже захочется in-process транскрипции и стриминга прогресса:**
- Можно перейти на native-биндинг, но заложить @electron/rebuild в CI и зафиксировать версию Electron

**Если юзер целиком в облаке (нет мощного железа):**
...
---
*Stack research for: cross-platform Electron mp4→transcript→LLM desktop app*
*Researched: 2026-05-27*
